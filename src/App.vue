      ...
      <md-list>
        <md-list-item>
          ...
          <md-button
            ...
            @click="createInventoryItem(inventoryItem)"
          >
    ...
    <md-table v-model="inventory" md-sort="name" md-sort-order="asc">
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        ...
        <md-table-cell md-label="Edit">  
          <md-button class="md-icon-button md-accent" @click="editInventoryItem(item)">
...
export default {
  ...
  methods: {
    list() {
      axios({ method: "GET", url: "http://localhost:8080/getInventoryList" }).then(
    ...
    deleteItem(id) {
      if (confirm("Are you sure ?")) {
        axios({
          method: "DELETE",
          url: "http://localhost:8080/deleteInventoryItemById",
          params: { id: id }
    ...
    createInventoryItem() {
      if(!this.inventoryItem.Id){
        axios
          .post("http://localhost:8080/createInventoryItem", this.inventoryItem)
          .then(
            response => (
    ...
    updateInventoryItem(){
      axios
        .patch("http://localhost:8080/updateInventoryItem", this.inventoryItem)
        .then(
          response => (
            console.log(response),
            this.cleanFields(),
